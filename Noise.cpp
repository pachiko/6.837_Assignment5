...
Vector3f Noise::getColor(const Vector3f & pos)
{
  ...
  float octaveNoise = amplitude*PerlinNoise::octaveNoise(pos, octaves);
  float noise = (sinf(frequency*pos[0] + octaveNoise) + 1.f)/2.f;
  return Vector3f::lerp(color[0], color[1], noise);
}
